...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      const data = req.body;
      if (!data?.fromName || !data?.toName || !data?.message) return res.status(400).json({ error: 'Missing required fields' });
      const key = `recognition/${Date.now()}_${sanitize(data.fromName)}.json`;
  ...
  if (req.method === 'GET') {
    try {
      const { blobs } = await list({ prefix: 'recognition/', token: process.env.BLOB_READ_WRITE_TOKEN });
